...
print("<center><h2><strong>Create a new group please : input data</strong></h2></center><br>");
...
<?php
open_database();
$sql_query="select gid,name,gname,tname,comment,homepath from groups where isadmin=\"0\" order by name asc ";
...
?>
<Form Name=getgroupinfo method=POST Action=add_group.php target=_self>
Please input the new data : <br>
<input type=button onClick='checkfill()' name=submit_data class=greenbutton value="Register Group">
<table border=1>
	<tr><td>Group Name (requered unique): </td><td><input type=text name=t_gname class=fontos value="<?=$_GET[gname]?>"></td></tr>
